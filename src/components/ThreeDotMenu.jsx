...
export const ThreeDotMenu = ({ data }) => {
  ...
  return (
    <div className="flex flex-col gap-4 text-md my-auto bg-[#f9f9f9] dark:bg-black absolute right-6 top-0 shadow-sm shadow-black rounded p-2">
      ...
      >
        <span
          className={`${foundInWatchLater ? 'material-icons' : 'material-icons-outlined'} mr-2`}
        >
      ...
      {token && pathname === '/liked' ? (
        ...
        >
          <span className="material-icons-outlined mr-2">block</span>Remove From Liked Videos
        </button>
      ) : null}
      {token && pathname === `/playlist/${playlistId}` && (
        ...
        >
          <span className="material-icons-outlined mr-2">block</span>Remove from current playlist
        </button>
      )}
