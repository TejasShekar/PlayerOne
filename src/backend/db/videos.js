/**
 * Videos Database can be added here.
 * You can add videos of your wish with different attributes
 * */

export const videos = [
  {
    _id: "yKmKjX5KpBc",
    category: "action",
    title: "10 GTA 5 Mods For A Wild Time In Los Santos",
    creator: "Rock Paper Shotgun",
    creatorID: "AKedOLR4JSBnLY_jE0bnQBgD1jHKHZlahkSfGXbzeA1m3A",
    views: "525,973",
    description:
      "We pick the best GTA 5 mods - or the GTA 5 mods that create the most chaos in Los Santos. Some GTA V mods improve the game, others just add a load of madness - if you are looking for a wild time in the city, these are our top pick of mods for GTA 5.",
    uploadDate: "May 27, 2020",
  },
  {
    _id: "1aM_zh-Kbl4",
    category: "action",
    title: "Cars Vs Cars Challenge 99.333% People Start Crying in GTA 5!",
    creator: "THE CASETOO",
    creatorID: "AKedOLSz-775q6Y5D46odFwqXfKwZ73mnj2ASdcTOlOU",
    views: "180,355",
    description:
      "Join the crew to Play GTA V With me. Bohot maza aane wale hai. Ekdum si badiya maza aajayega. Papa player banjayega. Ez. So, join my crew on GTA V fatafat.",
    uploadDate: "May 30, 2022",
  },
  {
    _id: "G7iA2ireN0c",
    category: "action",
    title: "Splinter Cell Blacklist - John Wick Style Kills",
    creator: "BareDickies",
    creatorID: "AKedOLSq3QyrYf6AKn-G8TtQU33cf3mMCF73dx3Ph4I6",
    views: "299,355",
    description:
      "Missions - Safehouse, American Consumtion. Difficulty - Realistic. Time to show some realistic OP John Wick Kills!",
    uploadDate: "Apr 11, 2020",
  },
  {
    _id: "SoDM9wwYpFw",
    category: "action",
    title: "Watch Dogs 2 - all DedSec cutscenes",
    creator: "Y2JakeMiami",
    creatorID: "AKedOLSFrRE-gipyYEtm-lRvA7uerwqYdCqvFxMpw5BGGw",
    views: "106,291",
    description:
      "Watch Dogs fans and fans of DedSec here all the cutscenes of DedSec in Watch Dogs 2. DedSec is a secret hacking group that serves their own brand of justice. I really enjoyed playing Watch Dogs 1 and 2 and the unique hacking story and its characters. Let me know if you want me to upload the DedSec cutscenes separately. DedSec has given you truth, do what you will - DedSec. Enjoy!",
    uploadDate: "Dec 23, 2016",
  },
  {
    _id: "Qs4OcQNOM34",
    category: "action",
    title: "Watch Dogs - E3 Intro",
    creator: "Ubisoft",
    creatorID: "AKedOLQjUa11x4gjSQcQEy09Zi5OvaxdRuwTk18hrDTt-T0",
    views: "612,469",
    description: "Everything is connected. Connection is power.",
    uploadDate: "Jun 5, 2012",
  },
  {
    _id: "YY2GX_7wR7Y",
    category: "racing",
    title: "Forza Horizon 5 First Drive",
    creator: "Forza",
    creatorID: "AKedOLTAVZ7CdsWlzWdcbxTHGnuD7LQOH78xSZjhQJHC",
    views: "7,134,349",
    description:
      "Presenting the first eight minutes of Forza Horizon 5, exploring our expansive version of Mexico. Featuring Forza Horizon 5’s cover cars, the Mercedes-AMG ONE and the 2021 Ford Bronco Badlands, and landscapes such as the active, snow-capped Gran Caldera volcano, a dust storm in the Mexican farmland, and dense jungle tree canopy. ",
    uploadDate: "Aug 24, 2021",
  },
  {
    _id: "8jiTNodDe-Y",
    category: "racing",
    title: "Need for Speed Heat Gameplay",
    creator: "Throneful",
    creatorID: "AKedOLRFpJvq-wBohC02mHXTpx_uhRit3eV204GWWoiHbg",
    views: "2,164,132",
    description: "Need For Speed never gets better than this crazy gameplay !",
    uploadDate: "Nov 5, 2019",
  },
  {
    _id: "uJT4y4YGIGs",
    category: "racing",
    title: "NFS Heat Mustang GT RTR - LOGITECH G29 gameplay",
    creator: "Fastay",
    creatorID:
      "YUT4suS0BZSemkBRckjv3XGwWfYNzP3SmeKXsB_wCiE5ufuqfDGEaybEhPUFk0p7t15BBHRx",
    views: "1,643,960",
    description:
      "Gameplay in Need for speed Heat with Rebuilding a FORD MUSTANG GT RTR using Logitech G29 steering wheel",
    uploadDate: "Jan 14, 2022",
  },
  {
    _id: "AXXtoCFwIXY",
    category: "racing",
    title: "Need for Speed Heat - BMW M3 GTR Legendary Edition 2005",
    creator: "Throneful",
    creatorID: "AKedOLRFpJvq-wBohC02mHXTpx_uhRit3eV204GWWoiHbg",
    views: "200,047",
    description:
      "Need for Speed Heat - BMW M3 GTR Legendary Edition 2005 - Open World Free Roam Gameplay",
    uploadDate: "Dec 5, 2019",
  },
  {
    _id: "vdz58E1MWBM",
    category: "racing",
    title: "Need For Speed Most Wanted 2012 - Get Wanted ",
    creator: "Electronic Arts",
    creatorID: "AKedOLTd5yUoktPB4xtedQ-U5UFDrdNk7wesxDRbRK3NEQ",
    views: "2,481,281",
    description:
      "With unprecedented freedom and power at your fingertips, experience the epic, high energy, vast open-world of Fairhaven where the possibilities of adventure are endless. Race against your rivals  at breakneck speeds across the varied city of Fairhaven.",
    uploadDate: "Sep 25, 2012",
  },
  {
    _id: "TgC0p_9Num8",
    category: "adventure",
    title: "Assassin's Creed Syndicate - Jack The Ripper",
    creator: "DKGames",
    creatorID: "AKedOLR4vlvC0Zp-ecCCOcA1RhSoCWlWUvM7Cg5kAWHlGw",
    views: "894,534",
    description:
      "Assassin's Creed Syndicate - Jack The Ripper Brutal Massacre & Fear Kills in Lambeth Asylum. One of Assassin's Creed more darker stories.",
    uploadDate: "Dec 24, 2020",
  },
  {
    _id: "WTBbwgsyxvg",
    category: "adventure",
    title: "Assassin’s Creed Syndicate E3 Cinematics",
    creator: "Ubisoft",
    creatorID: "AKedOLQjUa11x4gjSQcQEy09Zi5OvaxdRuwTk18hrDTt-T0",
    views: "10,622,343",
    description:
      "Establish Great Britain's fiercest gang with Jacob Frye as the leader to break the Templars' corrupt stanglehold on London.",
    uploadDate: "Jun 16, 2015",
  },
  {
    _id: "cHHsXsas41Y",
    category: "adventure",
    title: "Assassin's Creed: Brotherhood Gameplay",
    creator: "Ozone",
    creatorID:
      "lqWXpvf5cT7wjz_hJAj_8Y8OHj7adCxq46LIGxe7b0WWHRzwMsMyDh-2HdQtDFlRy4d7CWp5",
    views: "107,762",
    description:
      "Assassin's Creed - The Ezio Collection. One of the greatest and most respected character and Assasin of all the Assasins in the series. His compassion to grow the creed while helping people to overcome their fear and get independent shows his integrity. He's also know for his wise words. His famous quote is 'We work in the Dark to serve the Light' ",
    uploadDate: "Apr 10, 2021",
  },
  {
    _id: "0ppfJ9Mv71Y",
    category: "adventure",
    title:
      "13 Minutes of Caribbean Open-World Gameplay | Assassin's Creed 4 Black Flag",
    creator: "Assassin's Creed",
    creatorID: "AKedOLT52k4kP9eMbrjlikSuNOY0ZKo1tjjVGuwhx72ulg",
    views: "426,781",
    description:
      "Game Director Ashraf Ismail takes us deeper into the world of Assassin's Creed IV Black Flag with this new commented gameplay video. Embark on a journey with Edward and discover his daily life as a pirate and Assassin in this Open-World demo, in a quest for glory, fame and hidden treasures.",
    uploadDate: "Jul 23, 2013",
  },
  {
    _id: "EHAu6tfV4t8",
    category: "adventure",
    title: "Shadow of the Tomb Raider Gameplay",
    creator: "FA Gamez",
    creatorID: "AKedOLSHOGINwIkQ3Hv3AI9jthTVQjKbKxugkaFGkaUeMw",
    views: "1,902,072",
    description:
      "In Shadow of the Tomb Raider, Lara must master a deadly jungle, overcome terrifying tombs, and persevere through her darkest hour. As she races to save the world from a Maya apocalypse, Lara must become the Tomb Raider she is destined to be. Survive and Thrive: Master the jungle to survive.",
    uploadDate: "Jan 7, 2021",
  },
  {
    _id: "u07GzgHIV94",
    category: "adventure",
    title: "Assassin's Creed: Revelations Gameplay",
    creator: "Ozone",
    creatorID:
      "lqWXpvf5cT7wjz_hJAj_8Y8OHj7adCxq46LIGxe7b0WWHRzwMsMyDh-2HdQtDFlRy4d7CWp5",
    views: "227,702",
    description:
      "Assassin's Creed - The Ezio Collection. One of the greatest and most respected character and Assasin of all the Assasins in the series. His compassion to grow the creed while helping people to overcome their fear and get independent shows his integrity. He's also know for his wise words. His famous quote is 'We work in the Dark to serve the Light' ",
    uploadDate: "Nov 9, 2021",
  },
  {
    _id: "fJUKJs4eN3w",
    category: "sports",
    title: "MotoGP 2022 - Craziest Track on Planet",
    creator: "Joy of Gaming",
    creatorID: "AKedOLQ1mxKeLdKN16zd3v6xIB6_hh6dtzpXtSS1-fSDaw",
    views: "890,731",
    description: "Everything is connected. Connection is power.",
    uploadDate: "Apr 21, 2022",
  },
  {
    _id: "NB8PWxY7Jds",
    category: "sports",
    title: "F1 2020 Gameplay Monaco Lewis Hamilton",
    creator: "Tiametmarduk",
    creatorID:
      "kMWgmFXm5Ijs-sKqEGcdJoLSrfNztnTEJRAuarClWKSgfkZHtxvhAB3p7BvaSaf3vg0HVJP-",
    views: "2,403,888",
    description:
      "F1 2020 Monaco 78 Lap Race: No Assists, Simulation Damage and 110% AI and starting from Last Place. What more could you want?",
    uploadDate: "Jul 14, 2020",
  },
  {
    _id: "ygEtA8FMfwM",
    category: "sports",
    title: "F1 22 Gameplay: FIRST EVER RACE LAPS!",
    creator: "Aarava",
    creatorID: "AKedOLRYN0_zySHdNZOiDLFu9UxLQKsCrwBY1I2tSwRX_Q",
    views: "599,814",
    description:
      "Exclusive Direct-Feed F1 22 PC Gameplay, talking the new Handling & Tyre Model, New Manual Grid Spot Formation Lap, Manual Pitstops, & our very first laps ever of the Miami GP! Thanks to EA Creator Network",
    uploadDate: "May 5, 2022",
  },
  {
    _id: "SBCEIjVG4bY",
    category: "sports",
    title: "F1 2021 - Race in Brazil in Verstappen's Red Bull",
    creator: "Beatdown Racing x Veloce",
    creatorID: "AKedOLTzJRzIlRESKblj2Dvv5sKlg8Ey4hJktpkMyNuT",
    views: "453,397",
    description:
      "Check out this brand new video of F1 2021 gameplay, containing a full 100% race of the Brazilian Grand Prix! This Formula 1 race has been recorded by Beatdown Racing on the next-gen PS5 console with the AI driver level set to 95, a.k.a. Legend difficulty. Today we'll be racing at the Autódromo José Carlos Pace - also known as Interlagos - in São Paulo, Brazil using Max Verstappen's Red Bull RB16B!",
    uploadDate: "Nov 11, 2021",
  },
];
